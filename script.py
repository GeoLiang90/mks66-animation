...
def second_pass( commands, num_frames ):
    ...
    for command in commands:
        if command['op'] == 'vary':
            frameTotal = (command['args'][1] - command['args'][0])
            move = (command['args'][3] - command['args'][2])/frameTotal
            i = int(command['args'][0])
            #print(command['args'])
            #print(frameTotal)
            while i < int(command['args'][1]) + 1:
                frames[int(i)][command['knob']] = command['args'][2] + move * (i - int(command['args'][0]))
                #print(move * (i))
                i += 1
    #print(frames)
    return frames
...
def run(filename):
    ...
    for frame in range(int(num_frames)):
        ...
        for command in commands:
            ...
            if c in transformations and not(command['knob'] == None):
                knob_name = command['knob']
                print(symbols)
                symbols[knob_name] = ['knob',frames[frame][knob_name]]
                print(symbols)
                knob_value = symbols[knob_name][1]
            if c == 'box':
